...
func Send2DingTalk(content []common.Ready2Send, isRecover bool, sendTime string, url string, secret string) {
	defer func() {
		if e := recover(); e != nil {
			...
			gobal.GetLogger().Errorf("Panic in Send2DingTalk: %v\n%s", e, buf)
		}
	}()
...
func post2DingTalk(url string, secret string, msg *DingTalkMsg) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Parse the url: %s err: %v\n", url, err))
		return
	}
	qs := addr.Query()
	qs.Set("timestamp", t)
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Marshal the dingTalk msg: %v err: %v\n", msg, err))
		return
	}
...
func dingTalkMsg2String(content common.Ready2Send, sendTime string, isRecover bool) string {
	...
	if !isRecover {
		msg = append(msg, fmt.Sprintf("[点击确认告警](%s/alerts_confirm/%d?start=%d)", gobal.GetAlterGatewayConfig().Send.WebUrl, content.RuleId, content.Start))
	}
